    ...
    while True:
        ...
        if ser_air.in_waiting:
            ...
            while True:
                ...
                waypoint_coord = redbull[waypoint_idx]
                ...
                if animal_cnt >= 1:
                    if waypoint_idx == last_result_idx:
                        continue
                    last_result_idx = waypoint_idx

                    animal_name = ANIMAL_ID_MAP.get(animal_cls, "未知")
                    record_text = current_coord_text or "航点%d" % waypoint_idx

                    if animal_cls == 0xFF:
                        record_text += ",无动物"
                        print("航点 %d 检测完成：无动物" % waypoint_idx)
                    elif animal_name != "未知":
                        animal_key = str(animal_cls)
                        animal_totals[animal_key] += animal_cnt
                        screen_id = animal_screen_ids.get(animal_name, 16)
                        send_to_screen('page1.t%d.txt="%s"' % (screen_id, animal_totals[animal_key]))
                        record_text += ",%s:%d" % (animal_name, animal_cnt)
                        print("航点 %d 检测完成：%s x %d" % (waypoint_idx, animal_name, animal_cnt))
                    else:
                        record_text += ",未知:%d" % animal_cnt
                        print("航点 %d 收到未知动物类别：0x%02X x %d" % (waypoint_idx, animal_cls, animal_cnt))

                    display_line += 1
                    command = 'page0.t%d.txt="%s"' % (display_line, record_text)
                    print("显示航点检测记录：%s" % command)
                    send_to_screen(command)

                if waypoint_idx >= total_waypoints - 1 and animal_cnt >= 1:
                    print("所有航点已完成")
